 ...
 * 使用时Activity需要继承ConditionWithListActivity，同时布局文件应该为 include_list_with_condition
 */
public abstract class ConditionWithListActivity<P extends BasePresenter> extends BasicBusinessActivity<P> {

    @BindView(R2.id.sv_listWithCondition_scrollView)
    MNScrollView mScrollView;//滚动布局，该布局包裹了条件列表和数据列表
    /**
     * 显示条件的列表的ListView
     */
