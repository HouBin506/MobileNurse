...
/**
 * 底部选择导航栏的条目
 * Created by HouBin on 2017/12/19.
 */
...
public class BottomBarTab extends FrameLayout {
    ...
    public BottomBarTab(Context context, @Nullable AttributeSet attrs, @DrawableRes int icon, CharSequence title) {
        this(context, attrs, 0, icon, title);
    }

    public BottomBarTab(Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr, @DrawableRes int icon, CharSequence title) {
...
//        int size = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 27, getResources().getDisplayMetrics());
        //选择dimen中的尺寸，为了适配屏幕大小
        int size = ArmsUtils.getDimens(mContext, R.dimen.dp_25);
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(size, size);
        mIcon.setImageResource(icon);
        mIcon.setLayoutParams(params);
        //TODO 因为本项目没有切图标，所以没有标准的图标用来设置选择背景
//        mIcon.setColorFilter(ContextCompat.getColor(context, R.color.tab_unselect));
        ...
        //选择dimen中的尺寸，为了适配屏幕大小
        mTvTitle.setTextSize(ScreenUtils.px2sp(mContext, ArmsUtils.getDimens(mContext, R.dimen.text_content_level_6)));
        ...
        mTvUnreadCount.setBackgroundResource(R.drawable.shape_msg_bg_bubble);
    ...
    public void setSelected(boolean selected) {
        ...
        if (selected) {
//            mIcon.setColorFilter(ContextCompat.getColor(mContext, R.color.green_selected_bg));
            mTvTitle.setTextColor(ContextCompat.getColor(mContext, R.color.green_selected_bg));
        } else {
