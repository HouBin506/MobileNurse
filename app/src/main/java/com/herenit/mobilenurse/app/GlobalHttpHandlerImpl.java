...
public class GlobalHttpHandlerImpl implements GlobalHttpHandler {
    ...
    public Response onHttpResultResponse(@Nullable String httpResult, @NonNull Interceptor.Chain chain, @NonNull Response response) {
        ...
        if (url.contains(Api.LOGIN_PATH)) {//保存cookie
            List<String> cookieList = response.headers("Set-Cookie");
            ConfigSp.getInstance().setCookie(cookieList);
//            if (cookieList != null && !cookieList.isEmpty()) {
//                StringBuffer cookieBuf = new StringBuffer();
//                for (int x = 0; x < cookieList.size(); x++) {
//                    String cookie = cookieList.get(x);
//                    if (x != (cookieList.size() - 1))
//                        cookieBuf.append(cookie + "; ");
//                    else
//                        cookieBuf.append(cookie);
//                }
//                ConfigSp.getInstance().setCookie(cookieBuf.toString());
//            }
        }
        if (!TextUtils.isEmpty(httpResult) && RequestInterceptor.isJson(response.body().contentType())) {
            try {
                Result result = ArmsUtils.obtainAppComponentFromContext(context).gson().fromJson(httpResult, Result.class);
                if (result != null && !TextUtils.isEmpty(result.getMessage()))
                    Timber.w("Result ------> " + result.getMessage());
            } catch (Exception e) {
                e.printStackTrace();
                return response;
            }
    ...
    public Request onHttpRequestBefore(@NonNull Interceptor.Chain chain, @NonNull Request request) {
        ...
        if (!url.contains(Api.LOGIN_PATH)) {//非登录
            ...
            return chain.request().newBuilder().header("Cookie", cookieSb.substring(0, cookieSb.length() - 1)).build();
        } else {
            return request;
        }
    }
